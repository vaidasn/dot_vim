...
git config --global alias.edit '!e() { [[ -n $1 ]] && '`
                               `'{ exec gvim -n --servername $(git rev-parse --show-toplevel) --remote-tab-silent "$@" & } || '`
                               `'exec gvim -n --servername $(git rev-parse --show-toplevel) & }; e'
